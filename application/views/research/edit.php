<form action="<?= site_url('research/update/' . $research['id']) ?>" method="POST" enctype="multipart/form-data" id="form-research">
    <?= _csrf() ?>
    <div class="card mb-3">
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2" name="lecturer" id="lecturer" required data-placeholder="Select lecturer">
                    <option value="">-- Select lecturer --</option>
                    <?php foreach ($lecturers as $lecturer): ?>
                        <option value="<?= $lecturer['id'] ?>"<?= set_select('lecturer', $lecturer['id'], $lecturer['id'] == $research['id_lecturer']) ?>>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="research_title" name="research_title" maxlength="500" required
                          placeholder="Enter research title"><?= set_value('research_title', $research['research_title']) ?></textarea>
                <?= form_error('research_title') ?>
            </div>
            <div class="form-group">
                <label for="research_type">Jenis Penelitian</label>
                <textarea class="form-control" id="research_type" name="research_type" maxlength="100" required
                          placeholder="Enter research research_type"><?= set_value('research_type', $research['research_type']) ?></textarea>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="source_of_funds" name="source_of_funds" maxlength="200" required
                        value="<?= set_value('source_of_funds', $research['source_of_funds']) ?>" placeholder="Sumber dana">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2" name="year" id="year" required data-placeholder="Select year">
                    ...
                    <?php foreach ($years as $year): ?>
                        <option value="<?= $year?>"<?= set_select('year', $year, $research['year'] == $year) ?>>
                            <?= $year ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="journal_accreditation" name="journal_accreditation" maxlength="200" required
                        value="<?= set_value('journal_accreditation', $research['journal_accreditation']) ?>" placeholder="Akreditasi jurnal">
            ...
            <div class="form-group">
                ...
                <input type="url" class="form-control" id="journal_link" name="journal_link" maxlength="500" required
                        value="<?= set_value('journal_link', $research['journal_link']) ?>" placeholder="Link jurnal">
                <?= form_error('journal_link') ?>
            </div>
            <div class="form-group">
                <label for="description">Description</label>
                <textarea class="form-control" id="description" name="description" maxlength="500"
                          placeholder="Enter description"><?= set_value('description', $research['description']) ?></textarea>
                <?= form_error('description') ?>
            </div>
            <div class="d-flex justify-content-between mt-3">
                <button onclick="history.back()" type="button" class="btn btn-light">Back</button>
                <button type="submit" class="btn btn-primary" data-toggle="one-touch" data-touch-message="Updating...">Update Research</button>
            </div>
        </div>
    </div>
</form>
